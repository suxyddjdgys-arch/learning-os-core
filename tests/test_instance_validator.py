...
class InstanceValidationTests(unittest.TestCase):
    ...
    def test_valid_state_documents_schema_03_pass(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref=self.handoff_ref_ok())
        self.assert_pass()
    ...
    def test_additive_extension_pass(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(nodes={f"{DOMAIN}.local-extension": {"title": "Local", "kind": "application"}}))
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.local-extension"), provenance=self.valid_new_provenance(), handoff_ref=self.handoff_ref_ok())
        self.assert_pass()

    def test_new_local_domain_pass(self):
        write_yaml(self.instance, "curriculum/local/new-domain/curriculum.yaml", {"schema_version": "0.3", "document_type": "curriculum", "domain": {"id": "new-domain", "title": "New Domain"}, "curriculum_version": "1.0", "nodes": {"new-domain.root": {"title": "Root", "kind": "concept"}}, "edges": [], "aliases": {}})
        write_full_state(self.instance, curriculum_refs=self.valid_refs("new-domain.root", "new-domain"), provenance=[{"domain": "new-domain", "curriculum_version": "1.0"}], handoff_ref=self.handoff_ref_ok())
        self.assert_pass()

    def test_new_local_probe_pass(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(nodes={f"{DOMAIN}.probe-node": {"title": "Probe Node", "kind": "skill"}}, probes=[{"id": f"{DOMAIN}.probe-1", "prompt": "synthetic probe"}]))
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.probe-node"), provenance=self.valid_new_provenance(), handoff_ref=self.handoff_ref_ok())
        self.assert_pass()
    ...
    def test_new_provenance_pass(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(nodes={f"{DOMAIN}.local-extension": {"title": "Local", "kind": "application"}}))
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_new_provenance(), handoff_ref=self.handoff_ref_ok())
        self.assert_pass()
    ...
    def test_cli_instance_surface_pass(self):
        ...
        result = subprocess.run([sys.executable, str(CORE_REPO_ROOT / "scripts/validate_learning_os.py"), str(self.instance), "--instance", "--core-snapshot", str(self.core), "--deployment-binding", str(ctx_file)], capture_output=True, text=True)
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)

    def test_fail_state_schema_04(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref=self.handoff_ref_ok(), knowledge_schema="0.4")
        self.assertIn("instance.state_schema_unsupported", self.errors())

    def test_fail_unsupported_state_schema(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref=self.handoff_ref_ok(), knowledge_schema="0.5")
        self.assertIn("instance.state_schema_unsupported", self.errors())
    ...
    def test_fail_lineage_control_in_instance(self):
        write_yaml(self.instance, "runtime/lineages/learning-os-design.yaml", {"schema_version": "0.3", "document_type": "lineage_control", "lineage": {"id": "learning-os-design"}, "active_generation": 8, "pending_handoff": None})
        errors = self.errors(); self.assertIn("instance.private_lineage", errors); self.assertIn("instance.forbidden_document", errors)

    def test_fail_deployment_authority_fields(self):
        config = instance_config_doc(); config["deployment_epoch"] = 2; write_yaml(self.instance, "config/instance.yaml", config)
        self.assertIn("instance.authority_key", self.errors())

    def test_fail_core_node_collision(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(nodes={f"{DOMAIN}.foundation": {"title": "Shadow", "kind": "concept"}}))
        self.assertIn("instance.node_collision", self.errors())

    def test_fail_core_edge_collision(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(edges=[{"id": f"{DOMAIN}.edge-1", "relation": "extends", "strength": "weak"}]))
        self.assertIn("instance.edge_collision", self.errors())

    def test_fail_capability_override(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(capability_profiles={"conceptual_core": {"transfer": "not_expected"}}))
        self.assertIn("instance.capability_override", self.errors())

    def test_fail_delete_tombstone_core(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(nodes={f"{DOMAIN}.foundation": {"tombstone": True}}))
        self.assertIn("instance.core_delete", self.errors())

    def test_fail_core_alias_redirect(self):
        write_yaml(self.instance, "curriculum/extensions/base-extension.yaml", extension_doc(aliases={f"{DOMAIN}.alias": f"{DOMAIN}.application"}))
        self.assertIn("instance.alias_redirect", self.errors())

    def test_fail_same_id_different_content(self):
        write_yaml(self.instance, "curriculum/local/base-domain/curriculum.yaml", {"schema_version": "0.3", "document_type": "curriculum", "domain": {"id": DOMAIN, "title": "Shadow Domain"}, "curriculum_version": "9.9", "nodes": {f"{DOMAIN}.other": {"title": "Other", "kind": "concept"}}, "edges": [], "aliases": {}})
        self.assertIn("instance.domain_shadow", self.errors())

    def test_fail_unresolved_semantic_reference(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.nonexistent"), provenance=self.valid_legacy_provenance(), handoff_ref=self.handoff_ref_ok())
        self.assertIn("reference.curriculum_node", self.errors())

    def test_fail_traversal_handoff_ref(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref="../../outside/secret.yaml")
        self.assertIn("instance.ref_traversal", self.errors())

    def test_fail_absolute_handoff_ref(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref="/etc/passwd")
        self.assertIn("instance.ref_absolute", self.errors())

    def test_fail_cross_plane_reference(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref="protocol/runtime-core.md")
        self.assertIn("instance.ref_cross_plane", self.errors())

    def test_fail_unknown_document_type(self):
        write_yaml(self.instance, "learner/notes.yaml", {"schema_version": "0.3", "document_type": "mystery_document"})
        self.assertIn("instance.unknown_document_type", self.errors())

    def test_fail_malformed_evidence(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=self.valid_legacy_provenance(), handoff_ref=self.handoff_ref_ok(), evidence_extra={"classification_confidence": "high"})
        self.assertIn("evidence.legacy_confidence", self.errors())

    def test_fail_ambiguous_provenance_form(self):
        write_full_state(self.instance, curriculum_refs=self.valid_refs(f"{DOMAIN}.foundation"), provenance=[{"domain": DOMAIN, "curriculum_version": BASE_VERSION, "base_version": BASE_VERSION, "extension_revision": 1}], handoff_ref=self.handoff_ref_ok())
        self.assertIn("instance.provenance_ambiguous", self.errors())

    def test_fail_deployment_binding_unknown_key(self):
        ctx = deployment_binding(); ctx["live_endpoint"] = "https://github.example"; self.assertIn("instance.deployment_binding_keys", self.errors(ctx=ctx))

    def test_fail_deployment_binding_live_type(self):
        ctx = deployment_binding(); ctx["context_type"] = "live"; self.assertIn("instance.deployment_binding_type", self.errors(ctx=ctx))

    def test_fail_deployment_binding_bad_commit(self):
        ctx = deployment_binding(); ctx["core_commit"] = "not-a-commit"; self.assertIn("instance.deployment_binding_commit", self.errors(ctx=ctx))

    def test_fail_deployment_binding_missing_key(self):
        ctx = deployment_binding(); del ctx["epoch"]; self.assertIn("instance.deployment_binding_missing", self.errors(ctx=ctx))
    ...
    def test_fail_instance_config_core_override(self):
        config = instance_config_doc(); config["chat_routing"] = {"modes": ["learning"]}; write_yaml(self.instance, "config/instance.yaml", config)
        self.assertIn("instance.core_override", self.errors())

    def test_fail_instance_product_mismatch(self):
        config = instance_config_doc(); config["product"]["id"] = "other-os"; write_yaml(self.instance, "config/instance.yaml", config)
...
class InstanceLearningHandoffTargetIntegrityTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        core_td = tempfile.TemporaryDirectory(); inst_td = tempfile.TemporaryDirectory()
        self.addCleanup(core_td.cleanup); self.addCleanup(inst_td.cleanup)
        self.core = make_core(Path(core_td.name)); self.instance = make_instance(Path(inst_td.name))

    def seed(self, handoff_ref=HANDOFF) -> None:
        write_full_state(self.instance, curriculum_refs=[{"type": "curriculum_node", "domain": DOMAIN, "id": f"{DOMAIN}.foundation"}], provenance=[{"domain": DOMAIN, "curriculum_version": BASE_VERSION}], handoff_ref=handoff_ref)
    ...
    def assert_pass(self) -> None:
        self.assertEqual([], [f.render() for f in validate_instance(self.instance, self.core, deployment_binding()) if f.severity == "error"])
    ...
    def mutate_handoff(self, **over) -> None:
        data = self.read_yaml(self.HANDOFF); data.update(over); write_yaml(self.instance, self.HANDOFF, data)
    ...
    def symlink_or_skip(self, link: Path, target: Path | str, *, target_is_directory: bool = False) -> None:
        try: link.symlink_to(target, target_is_directory=target_is_directory)
            ...
            if getattr(exc, "winerror", None) == 1314: self.skipTest("Windows host does not grant symbolic-link creation privilege")
            raise

    def test_valid_completed_handoff(self): self.seed(); self.assert_pass()

    def test_valid_pending_successor_handoff_without_n_plus_one_assumption(self):
        self.seed(); self.mutate_handoff(to_generation=3); runtime = self.read_yaml(self.RUNTIME)
        runtime["active_generation"] = 1; runtime["pending_successor"] = {"generation": 3}; runtime["generations"] = {"1": {"lifecycle": "handoff_pending", "handoff_ref": self.HANDOFF}}
        write_yaml(self.instance, self.RUNTIME, runtime); self.assert_pass()

    def test_fail_allowed_prefix_final_symlink_escape(self):
        self.seed(); target_data = self.read_yaml(self.HANDOFF); outside_td = tempfile.TemporaryDirectory(); self.addCleanup(outside_td.cleanup)
        outside = Path(outside_td.name) / "handoff.yaml"; write_yaml(Path(outside_td.name), "handoff.yaml", target_data); link = self.instance / self.HANDOFF; link.unlink(); self.symlink_or_skip(link, outside)
        self.assertIn("branch.handoff_ref", self.errors())

    def test_fail_allowed_prefix_directory_component_symlink_escape(self):
        self.seed(); target_data = self.read_yaml(self.HANDOFF); lineage_dir = (self.instance / self.HANDOFF).parent; (lineage_dir / "C01-to-C02.yaml").unlink(); lineage_dir.rmdir()
        outside_td = tempfile.TemporaryDirectory(); self.addCleanup(outside_td.cleanup); outside = Path(outside_td.name); write_yaml(outside, "C01-to-C02.yaml", target_data); self.symlink_or_skip(lineage_dir, outside, target_is_directory=True)
        self.assertIn("branch.handoff_ref", self.errors())

    def test_fail_instance_owned_existing_non_handoff_target(self): self.seed("topics/modern-language-models/plan.yaml"); self.assertIn("branch.handoff_ref_target", self.errors())
    def test_fail_wrong_branch(self): self.seed(); self.mutate_handoff(branch_id="other-branch"); self.assertIn("branch.handoff_ref_identity", self.errors())
    def test_fail_wrong_lineage(self): self.seed(); self.mutate_handoff(lineage_id="other-lineage"); self.assertIn("branch.handoff_ref_identity", self.errors())
    def test_fail_wrong_topic(self): self.seed(); self.mutate_handoff(topic="other-topic"); self.assertIn("branch.handoff_ref_identity", self.errors())
    def test_fail_wrong_from_generation(self): self.seed(); self.mutate_handoff(from_generation=2); self.assertIn("branch.handoff_ref_identity", self.errors())
    def test_fail_wrong_to_generation(self): self.seed(); self.mutate_handoff(to_generation=3); self.assertIn("branch.handoff_ref_identity", self.errors())
    def test_fail_explicit_empty_handoff_ref(self): self.seed(""); self.assertIn("instance.ref_invalid", self.errors())

    def test_fail_missing_handoff_target(self):
        self.seed("topics/modern-language-models/subtopics/language-modeling/handoffs/modern-language-models-main-lineage/C09-to-C10.yaml"); self.assertIn("branch.handoff_ref", self.errors())

    def test_benign_github_workflow_metadata_is_allowed(self):
        write_file(self.instance, ".github/workflows/validate-instance.yml", "name: validate-instance\non: push\n"); errors = self.errors(); self.assertNotIn("instance.core_owned_top", errors); self.assertNotIn("instance.top_level", errors); self.assertEqual(set(), errors)
